...
test "zalgebra.Vec2.negate" {
    var a = Vec2.set(5);
    var b = Vec2.set(-5);
    try expectEqual(Vec2.eql(a.negate(), b), true);
}
...
test "zalgebra.Vec2.normalize" {
    var a = Vec2.new(1.5, 2.6);
    try expectEqual(Vec2.eql(a.norm(), Vec2.new(0.499722480, 0.866185605)), true);
}

test "zalgebra.Vec2.sub" {
    var a = Vec2.new(1, 2);
    var b = Vec2.new(2, 2);
    try expectEqual(Vec2.eql(Vec2.sub(a, b), Vec2.new(-1, 0)), true);
}

test "zalgebra.Vec2.add" {
    var a = Vec2.new(1, 2);
    var b = Vec2.new(2, 2);
    try expectEqual(Vec2.eql(Vec2.add(a, b), Vec2.new(3, 4)), true);
}

test "zalgebra.Vec2.scale" {
    var a = Vec2.new(1, 2);
    try expectEqual(Vec2.eql(Vec2.scale(a, 5), Vec2.new(5, 10)), true);
}
...
test "zalgebra.Vec2.lerp" {
    ...
    try expectEqual(Vec2.eql(Vec2.lerp(a, b, 0.5), Vec2.new(0.0, 5.0)), true);
}
...
test "zalgebra.Vec2.min" {
    ...
    try expectEqual(Vec2.eql(Vec2.min(a, b), Vec2.new(-10.0, -2.0)), true);
}
...
test "zalgebra.Vec2.max" {
    ...
    try expectEqual(Vec2.eql(Vec2.max(a, b), Vec2.new(10.0, 5.0)), true);
}

test "zalgebra.Vec2.abs" {
    {
        const vec = Vec2.new(-42, -43);
        const expected = Vec2.new(42, 43);
...
test "zalgebra.Vec2.fromSlice" {
    ...
    try expectEqual(Vec2.eql(Vec2.fromSlice(&array), Vec2.new(2, 4)), true);
}
...
test "zalgebra.Vec2.cast" {
    ...
    try expectEqual(Vec2_f64.eql(c.cast(f64), d), true);

    const e = Vec2_i32.new(3, 6);
    const f = Vec2.new(3.0, 6.0);

    try expectEqual(Vec2.eql(e.cast(f32), f), true);

    const g = Vec2.new(3.0, 6.0);
    const h = Vec2_i32.new(3, 6);

    try expectEqual(Vec2_i32.eql(g.cast(i32), h), true);
}
